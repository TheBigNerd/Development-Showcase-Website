...
interface AuroraBackgroundProps extends React.HTMLProps<HTMLDivElement> {
  children: ReactNode;
  gradientStyle?: "corner" | "full" | "gentle-full" | "center";
  opacity?: number; // Control brightness
}
 
export const AuroraBackground = ({
  className,
  children,
  gradientStyle = "corner",
  opacity = 0.5, // Default opacity
  ...props
}: AuroraBackgroundProps) => {
  // Determine mask based on style
  const getMaskClass = () => {
    switch (gradientStyle) {
      case "corner":
        return `[mask-image:radial-gradient(ellipse_at_100%_0%,black_10%,var(--transparent)_70%)]`;
      case "full":
        return ``; // No mask
      case "gentle-full":
        return `[mask-image:radial-gradient(ellipse_at_center,black_80%,var(--transparent)_100%)]`;
      case "center":
        return `[mask-image:radial-gradient(circle_at_center,black_40%,var(--transparent)_80%)]`;
      default:
        return `[mask-image:radial-gradient(ellipse_at_100%_0%,black_10%,var(--transparent)_70%)]`;
    }
  };
